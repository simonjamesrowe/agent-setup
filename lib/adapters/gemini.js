...
module.exports = {
  ...
  // Gemini's own re-authorization command is unverified as of 2026-08-25. This hint is inert
  // today because checkRegistered() above never reports needsAuth (~/.gemini/settings.json
  // carries no auth state), but it stays generic rather than inventing a command in case that
  // changes and this branch becomes reachable for gemini.
  authHint: "sign in following gemini's own MCP authorization flow (exact command not verified as of 2026-08-25 — check gemini's own docs)",
};
