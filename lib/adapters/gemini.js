...
module.exports = {
  ...
  // Gemini signs an MCP server in with the `/mcp auth <server-name>` slash command inside an
  // interactive gemini session — there is no `gemini mcp login` CLI subcommand. Verified
  // 2026-08-25 against the installed gemini-cli 0.49.0: `gemini mcp --help` lists only
  // `add remove list enable disable`, while the bundled source at
  // /opt/homebrew/lib/node_modules/@google/gemini-cli/bundle carries the user-facing string
  // `Use /mcp auth <server-name> to authenticate.` and its own docs/tools/mcp-server.md
  // documents `/mcp auth serverName` under "Managing OAuth authentication".
  //
  // Reachability: this hint IS printed today, on the install path — provisionMcp notes a fresh
  // `mcp add` of a needsAuth server with the authHint of whichever adapter did the adding, and a
  // test in test/provisioner-mcp.test.js pins this exact value for gemini. Only the doctor
  // /`optional` branch is unreachable for gemini, because checkRegistered() above never reports
  // needsAuth (~/.gemini/settings.json carries no auth state).
  authHint: (serverName) => `run /mcp auth ${serverName} in an interactive gemini session`,
};
