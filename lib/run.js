...
async function run(args) {
  ...
  if (!skipMcp) results.push(...provisionMcp({ adapters, exec: realExec, check }));
  if (!skipPlugins) {
    const hasClaude = adapters.some((a) => a.key === 'claude');
    results.push(...(await provisionPlugins({ exec: realExec, check, yes: args.yes, prompt: makePrompt(args.yes), home, hasClaude })));
  }
