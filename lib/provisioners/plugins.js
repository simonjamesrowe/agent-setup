...
async function provisionModerne({ exec, check, yes, prompt, adapters }) {
  ...
  for (const a of supported) {
    ...
    const nowRegistered = isModerneRegistered(exec, a);
    finalRows.push(
      nowRegistered
        ...
        : {
          tool: a.key,
          status: 'failed',
          note: `mod config agent-tools ${a.key} install reported success but did not register the server — mod did not detect this agent`,
        }
    );
  }
...
module.exports = { provisionPlugins, findPlugin, moderneAuthStatus, moderneAuthRow };
