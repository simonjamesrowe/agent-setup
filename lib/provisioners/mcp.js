...
const MCP_SERVERS = [
  ...
  // simonrowe.dev's own MCP server, served by the Spring Boot backend at `/mcp` on the same host
  // as the public API — so it is up exactly when api.simonrowe.dev is up, and needs no local
  // process. It exposes the site's content as tools: getProfile, getJobs, getSkills,
  // searchBlogs / getRecentBlogs, searchNews, getUpcomingEvents, getCodeExamples, searchSite and
  // submitContactForm.
  //
  // Transport is streamable HTTP, matching the `type: 'http'` mcpAddArgs every adapter already
  // emits. Probed 2026-08-31: POST https://api.simonrowe.dev/mcp with an `initialize` request
  // answers 200 and `{"protocolVersion":"2025-06-18", ... "serverInfo":{"name":"mcp-server"}}`,
  // and `tools/list` over the returned `mcp-session-id` returns the ten tools above. The legacy
  // SSE paths (`/sse`, `/mcp/sse`) both 404, so there is no sse-transport variant to prefer.
  //
  // No `needsAuth`: the endpoint answers unauthenticated. `submitContactForm` is the one
  // side-effecting tool (it emails Simon) and the server rate-limits it to once per session
  // itself, so registering read/write costs nothing.
  { name: 'simonrowe-dev', type: 'http', url: 'https://api.simonrowe.dev/mcp' },
];
